...
Workflow提供了两种SSH调试模式：

**编译前SSH调试：** 勾选 `ssh: true` 可在编译前连接，用于检查配置和调试问题  
**自动失败调试：** 编译失败时自动启动SSH，无需额外配置

详见 [SSH_DEBUG_GUIDE.md](SSH_DEBUG_GUIDE.md) 获得详细的SSH调试教程和命令示例。
...
📖 **详细指南：** 查看 [SSH_DEBUG_GUIDE.md](SSH_DEBUG_GUIDE.md)

**快速开始：**
1. 勾选 `ssh: true` 在编译前进行SSH调试
2. 编译失败时会自动启动SSH连接
3. 使用提供的SSH命令连接到构建环境
4. 执行调试命令排查问题
...
- [SSH调试完整指南](SSH_DEBUG_GUIDE.md) - 详细的SSH调试教程和命令示例

## 💡 建议与最佳实践

1. **首次编译** - 建议先进行一次完整编译，验证流程无误
2. **缓存选项** - 启用缓存可大幅加速后续编译
3. **SSH调试** - 遇到问题时使用SSH调试可快速定位原因
4. **定时维护** - 定期检查Jell库更新以获得最新包
5. **版本管理** - 在GitHub Release中保存重要版本的配置
